...
__declspec(dllimport) int __stdcall WriteFile(void *file, const void *data, u32 size,
...
static void append_char(char value) {
    if (output_size < (u32)sizeof(output)) output[output_size++] = value;
}
...
static void append_hex32(u32 value) {
    ...
    int shift;
    append_text("0x");
    for (shift = 28; shift >= 0; shift -= 4) append_char(digits[(value >> shift) & 15U]);
}
...
static void field(const char *name, u32 value, int last) {
    append_char('"'); append_text(name); append_text("\":\""); append_hex32(value);
    append_char('"'); append_char(last ? '}' : ',');
}
...
static u32 parse_index(const char *command_line) {
    ...
    u32 value = 0;
    while (*command_line) {
        if (*command_line == ' ') last = command_line + 1;
        ++command_line;
    }
    while (*last >= '0' && *last <= '9') value = value * 10U + (u32)(*last++ - '0');
    return value;
}
...
static void emit(const struct matrix_case *test, const struct matrix_record *record) {
    u32 index;
    append_text("{\"schemaVersion\":1,\"name\":\""); append_text(test->name);
    append_text("\",\"encoding\":\"");
    for (index = 0; index < test->code_size; ++index) append_byte(test->code[index]);
    append_text("\",");
    field("case", record->case_id, 0); field("eax", record->eax, 0);
    field("ebx", record->ebx, 0); field("ecx", record->ecx, 0);
    field("edx", record->edx, 0); field("esiDelta", record->esi, 0);
    field("ediDelta", record->edi, 0); field("eflags", record->eflags, 0);
    for (index = 0; index < 8; ++index) {
        char name[] = "memory0"; name[6] = (char)('0' + index);
        field(name, record->memory[index], 0);
    }
    for (index = 0; index < 4; ++index) {
        char name[] = "xmm0_0"; name[5] = (char)('0' + index);
...
static void run_index(u32 index) {
    ...
    if (matrix_cases[index].use_memory_esi) input.esi = (u32)memory;
    code = (u8 *)VirtualAlloc((void *)0, 4096U, MEM_COMMIT_RESERVE, PAGE_EXECUTE_READWRITE);
    if (!code) ExitProcess(5U);
    ...
    if (!WriteFile(GetStdHandle(STD_OUTPUT_HANDLE), output, output_size, &written, (void *)0) ||
        written != output_size) ExitProcess(6U);
...
void start(void) {
    ...
    u32 current;
    if (index > MATRIX_CASE_COUNT) ExitProcess(4U);
